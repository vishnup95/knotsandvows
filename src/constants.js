export const API_HOST = process.env.API_HOST || 'https://prod.ahwanam.com';
export const API_PORT = process.env.API_PORT || '';

export const SUCCESS_DEFAULT = 'SUCCESS_DEFAULT';
export const FAILURE_DEFAULT = 'FAILURE_DEFAULT';
export const USER_LOCKED = 'USER_LOCKED';




export const GOOGLE_CLIENT_ID = '662359187269-cba1gqa2aa45sf174880uve3b8osdnl1.apps.googleusercontent.com';

export const REACT_APP_GA_TRACKING_ID='UA-139140156-1';
export const REACT_APP_FS_ORG_ID='J1FQ8';